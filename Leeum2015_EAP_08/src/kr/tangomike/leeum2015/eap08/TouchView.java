...
import kr.tangomike.leeum2015.eap08.TuioPoint;
...
public class TouchView extends SurfaceView implements SurfaceHolder.Callback {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (c != null) {
			...
			if(navX >= (bgX + backgroundImage.getWidth()) - centerMargin){ // right
				navX = bgX + backgroundImage.getWidth() - centerMargin;
			}
			if(navY <= bgY + centerMargin){ // top
				navY = bgY + centerMargin;
			}
			if(navY >= (bgY + backgroundImage.getHeight()) - centerMargin){ // bottom
				navY = bgY + backgroundImage.getHeight() - centerMargin;
			}
