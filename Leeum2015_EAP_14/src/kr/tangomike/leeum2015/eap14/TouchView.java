...
package kr.tangomike.leeum2015.eap14;
...
import kr.tangomike.leeum2015.eap14.TuioPoint;
...
public class TouchView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static final float maxNavScale = 525.5f;
	
	//background position
	private static final int bgX = 20;
	private static final int bgY = 81;
	
	//boundary limit 
	private static final int boundaryLeft = 20;
	private static final int boundaryTop = 81;
	private static final int boundaryRight = 1260;
	private static final int boundaryBottom = 672;
	...
	public TouchView(Context context, String oscIP, int oscPort, boolean drawAdditionalInfo, boolean sendPeriodicUpdates) {
		...
		backgroundImage = BitmapFactory.decodeResource(getResources(), R.drawable.eap_14_bg);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (c != null) {
			...
			{
				navY = boundaryTop + (navScale/16*9);
			}
			...
			if(navScale < minNavScale + navMargin/2){
				c.drawRect(navX - navScale - 20, navY - (navScale / 16*9) - 20, navX + navScale + 20, navY + (navScale / 16*9) + 20, touchAreaPaint);
			}
